...
def fetion(msg):
    ...
    page = jump.read();
    url = re.compile(r'<card id="start".*?ontimer="(.*?);').findall(page)[0]             #获取跳转链接
    arg_t = re.compile(r't=(\d*)').findall(page)[0]
    if url == '/im/login/login.action':                                                   #登录失败
        print 'Login Failed!'
    ...
    response = urllib2.urlopen(logout)                                                    #注销
